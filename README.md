...
> by the human in the browser simulator, the 210-test suite passes,
...
Positions are regenerated by a layout script that snaps the segments
onto a grid, not hand-edited: `PUT /api/boston/graph` with
`{"vertices": [{"id": "park_street", "x": 0.5, "y": 0.44}]}` moves
stations and the panel re-reads the file on its next frame, so an
arrangement that is written by hand will be overwritten the next time
the script runs.
...
GET  /api/boston/graph             the MBTA map graph the panel draws
PUT  /api/boston/graph             move stations: {vertices: [{id, x, y}]}
GET  /                               the page
